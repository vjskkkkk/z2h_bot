"""
sensex_monthly_backtest_v5.py
==============================
Monthly SENSEX backtest with all v5 Natenberg improvements.
...
# ── Change these ──────────────────────────────────────────────
BACKTEST_YEAR  = 2025
BACKTEST_MONTH = 12      # February

UNDERLYING  = "SENSEX"
EXCHANGE    = "BSE"
EXPIRY_WDAY = 3   # Thursday (changed from Friday/Tuesday, effective Sep 2025)
...
def main():
    ...
    print("\n" + "═"*76)
    print(f"  ZERO HERO v5 — SENSEX MONTHLY BACKTEST  |  {month_name}")
    ...
    print(f"  Expiry day excluded: Thursday (post Sep-2025 BSE change)")
    print("═"*76)

    token = get_access_token()
    days  = get_trading_days(BACKTEST_YEAR, BACKTEST_MONTH)

    print(f"\n  Trading days (ex-Thurs) : {len(days)}")
