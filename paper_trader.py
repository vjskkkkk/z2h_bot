"""
live/paper_trader.py  (v2 — handles BUY + SELL_NAKED + SELL_SPREAD)
"""
...
def can_trade(log):
    log = reset_daily(log)
    if log["open_trade"]:             return False, "❌ Trade already open"
...
def enter_trade(log, entry):
    ...
    if action == "BUY":
        ...
        premium_received = spread.get("net_credit", premium) if action == "SELL_SPREAD" else premium
        sl_price         = round(premium_received * 2, 2)  # 2× received = SL
        cost             = 0  # margin not deducted in paper mode
        ...
        trade = {
            ...
            "cost":            0,
            "sl_price":        sl_price,
            "target_price":    0,  # sellers exit at near-zero premium
            "trailing_sl":     sl_price,
            "partial_exited":  False,
            "spread":          spread,
            "status":          "OPEN",
...
def update_trade(log, current_price):
    ...
    if action == "BUY":
        ...
        # SL: current price exceeds 2× what we collected
        if current_price >= trade["sl_price"]:
            return _close_trade(log, current_price,
                                f"🛑 Sell SL hit — premium at ₹{current_price:.2f}")
        # Target: premium decayed to 20% of received — take profit
...
def get_daily_summary(log):
    ...
    for t in trades:
        ...
        lines.append(f"{icon} {t['id']} [{act}] {t['symbol']} | "
